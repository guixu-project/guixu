...
use crate::common::{
    UDFCategory, UDFDescriptor, UDFError, UDFId, UDFLimits, UDFMetadata, UDFResult, UFDCapabilities,
};
...
impl UDFServiceClient {
    ...
    pub async fn invoke_valuation(
        ...
        let mut request = self
            .client
            .post(format!("{}/valuation/{}", self.base_url, udf_id));
        if let Some(token) = &self.auth_token {
            request = request.header("Authorization", token);
        }
    ...
    pub async fn invoke_sampling(
        ...
        let mut request = self
            .client
            .post(format!("{}/sampling/{}", self.base_url, udf_id));
        if let Some(token) = &self.auth_token {
            request = request.header("Authorization", token);
        }
...
mod tests {
    ...
    fn test_remote_sampling_udf_metadata() {
        let udf = RemoteSamplingUDF::new("http://localhost:8080/sampling".to_string(), None);
        let metadata = udf.metadata();
