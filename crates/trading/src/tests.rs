...
mod tests {
    ...
    fn make_ctx(amount: f64, single: bool, batch: bool, fiat: bool, verify: bool) -> TransactionContext {
        TransactionContext {
    ...
    fn eip712_hash_is_deterministic() {
        let wallet = test_wallet();
        let to: alloy_primitives::Address = "0x1234567890abcdef1234567890abcdef12345678".parse().unwrap();
        let token: alloy_primitives::Address = "0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913".parse().unwrap();
    ...
    fn select_x402_for_micropayment() {
        ...
        assert!(matches!(router.select_protocol(&ctx), PaymentProtocol::X402));
    }
    ...
    fn select_mpp_for_session_batch() {
        ...
        assert!(matches!(router.select_protocol(&ctx), PaymentProtocol::StripeMpp));
    }
    ...
    fn select_escrow_for_large_verified() {
        ...
        assert!(matches!(router.select_protocol(&ctx), PaymentProtocol::Erc8183));
    }
