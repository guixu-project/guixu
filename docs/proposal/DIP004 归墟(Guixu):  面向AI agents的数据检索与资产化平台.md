20260324 [VLDB 2026 Demo Proposal] 归墟(Guixu):  面向AI agents的数据检索与资产化平台

VLDB 2026 DEMO Track DDL: 北京时间  2026/03/30 19:59:59
第一章: 背景与挑战
1.1 背景
AI Agent（自主智能体）正在从"对话助手"演进为"自主执行者" (例如ChatGPT --> OpenClaw) ——它们能够独立规划任务、调用工具、获取资源并交付结果。在这一演进过程中，数据是 Agent 完成任务的核心燃料。一个被要求"分析中国各省 GDP 增长趋势并预测 2026 年"的 Agent，必须自主完成以下步骤：定位相关经济数据集、评估数据质量、协商访问权限、验证数据完整性，最终将数据用于分析。
然而，当前的数据生态系统是为人类设计的。数据集散落在 Kaggle、HuggingFace、政府开放数据平台、企业内部数据库等孤岛中，每个平台有不同的 API、认证方式和访问模式。Agent 无法像人类浏览网页那样"逛"这些平台——它需要一个统一的、机器原生的协议来发现、评估、获取和验证数据。
...
本项目提出 Guixu——一个面向 AI Agent 的 P2P 数据检索与资产化平台，旨在构建一个去中心化的数据发现与交易网络，让 Agent 能够通过单一 MCP 接口完成从数据发现到数据消费的全链路。

归墟 (Guixu) —— 万水汇聚之所
《列子·汤问》中记载，在大海的最东边有一个无底之谷，叫做“归墟”。天下所有的水，无论是地上的河流还是天上的银河，最终都汇聚于此，且这里的水量永远保持平衡，不增不减。

一段话描述：任何人都可以利用Guixu的协议发布自己的数据集（无论免费还是收费），AI agents只需要安装Guixu的MCP即可用自然语言检索自己想要的数据集，Guixu会自动化根据用户预算（包括0预算）来完成数据集搜索到交付的p2p全流程，包括：1. 数据集检索（异构数据源或者数据库），2. 价值评估（按需评估），3. 数据验证（防止恶意攻击或虚假数据），4. 数据交易（全自动化+高频次+小额的去中心化交易+x402+Stripe MPP），5. p2p数据共享（bittorrent协议），6. 数据隐私保护（数据水印+版权保护）。最终目的是让Agents找到“满意”的数据集，让个人用户可以利用数据创造收益。

1.2 系统总览
Guixu 架构的核心组件：Agent 接口层、引擎层（四大引擎）、P2P 数据共享层。
[图片]
图 1：Guixu 系统架构总览

图 1 展示了 Guixu 的三层架构。顶层 MCP Server 暴露 7 个工具供 Agent 调用；中间层四大引擎（搜索、估值、交易、认证）处理核心逻辑；底层 P2P 共享层采用双平面设计，Metadata Plane (libp2p) 负责轻量级元数据路由，Data Plane (BitTorrent v2) 负责大文件高吞吐传输。
...
1.2.1 数据发布与检索：没有统一的数据发布和检索协议（挑战一）

问题描述： Agents 需要从无上限的异构数据源中发现满足特定任务需求的数据，但当前不存在统一的数据发布和检索协议，让任何人都可以通过一个协议发布自己的数据，P2P实现数据交付和贸易。

范式转变：就像Google Search一样，所有人都可以将网站托管到自己服务器上，被Google检索。未来，会有更多人或者agents将各种datasets托管在服务器或者自己PC上，被一种尚未出现的统一协议检索和发现，并资产化自己的datasets进而创造盈利。

产业现状：
- Google Dataset Search 索引了超过 3000 万个数据集（基于 schema.org/Dataset 标记），不仅中心化程度严重而且并非为ai agents原生设计，且不能数据贸易，用户必须依赖单一中心化平台展示数据，而不能使用更加隐私的p2p技术展示和交付数据。
...
核心空白： 没有一个协议能让 Agent 通过单次自然语言查询，同时搜索 P2P 网络中的私有数据集和 Kaggle/HuggingFace 等中心化平台的公开数据集，并返回统一格式的排序结果，并且实现P2P数据交付等一系列数据资产化流水线。


---

1.2.2 数据估值（Data Valuation）：Agents需要智能化在不同场景中按需使用高价值的数据（挑战二）

问题描述： Agent 在购买数据集前需要评估其价值，但当前缺少 Agent 可调用的自动化估值机制。直观的例子就是，agents解决解决图像分类问题时应当智能检索该任务的相关数据集，甚至检索到相关的agent memory。

产业现状：
- 现有数据市场（OpenDataBay、Snowflake Marketplace、Ocean Protocol）的价值估计方式极为粗糙：卖家自定价或 AMM 自动做市，没有基于数据内在价值的评估。
...
1.2.3 数据可验证性（Data Authentication）：防止数据被恶意篡改而引起的攻击（挑战三）

问题描述： Agent 获取的数据集可能被篡改、伪造或质量低劣，但当前缺少面向结构化数据集的密码学验证标准。直观的来说，agent memory若被投毒，agent本身就变成恶意攻击者，影响数据安全。若数据集被投毒，不仅影响agent完成任务的质量，还浪费用户预算。

产业现状：
- C2PA（Content Credentials） 为图片/视频提供了来源签名和篡改检测标准，但未覆盖结构化数据集（CSV/Parquet/数据库表）。
- Croissant（JSON-LD） 是 Kaggle 和 HuggingFace 采用的 ML 数据集元数据标准，但它是描述性的，元数据可以被伪造，没有密码学保证。
...
1.2.4 自动化数据交易（Data Trading）：高频次、小额交易、全自动化的数据贸易高效系统尚未出现（挑战四）
...
学术现状：
- ERC-8183 规范由 MetaMask、以太坊基金会、Google、Coinbase 工程师共同编写，2026 年 1 月上线以太坊主网，是目前最完整的 Agent 交易协议。
...
1.2.5 数据隐私保护（Data Privacy）：用户数据需要高效p2p检索甚至贸易（ 挑战五）
...
1.2.6 去中心化数据共享（Data Sharing）：去中心化数据共享网络尚未面向ai agents和数据贸易进行基础设施优化(挑战六)
...
2.1 各组件详细设计
2.1.1 P2P Data Sharing Layer（P2P 数据共享层）
...
延迟
Metadata Plane
libp2p (Kademlia DHT + GossipSub + mDNS + Noise)
元数据 JSON-LD (~1-2KB)
~5s (GossipSub) / ~2min (DHT)
Data Plane
BitTorrent v2 (BEP 52)
数据集分片 (Parquet, 256KB pieces)
取决于文件大小和 swarm
...
2.1.2 Data Search Engine（数据搜索引擎）
...
2.1.3 Data Authentication Engine（数据认证引擎）
...
L1 完整性
数据未被篡改
BitTorrent v2 Merkle Tree + CID 重算
自动、免费
L2 来源
谁发布的、何时发布
DID (did:key/did:ethr) + W3C VC + EAS 链上锚定
低 Gas (L2)
L3 属性
行数、schema、统计特征
Noir ZKP 电路（采样证明）
...
2.1.4 Data Trading Engine（数据交易引擎）
...
x402 (Coinbase)
单次微支付 < $0.01（预览采样）
USDC on Base L2, ~200ms
Stripe MPP
高频会话（搜索→预览→采样→购买）
SPT + USDC on Tempo, <1s
ERC-8183 Escrow
大额交易 > $1（需验证后释放）
USDC on Base L2, 验证后释放
...
2.1.5 Data Valuation Engine（数据估值引擎）
...
免费数据
从海量候选中选出最适合当前任务的
Task Fitness Score（schema 相关性 30% + 时间覆盖 20% + 信息增益 20% + 质量 15% + 新鲜度 10% + 去重价值 5%）
付费数据
判断是否值得花钱购买
ROI 评估（替代品分析 + Fast-DataShapley 边际价值 + 质量溢价）
Agent Memory/Skills
评估共享经验是否适合当前任务
能力图谱匹配（任务相关性 35% + 历史成功率 25% + 能力覆盖度 20% + 时效性 10% + 可迁移性 10%）
...
[图片]
图 2：Guixu 端到端工作流——免费数据 vs 付费数据双路径

图 2 展示了 Guixu 的双路径工作流。Agent 发出自然语言查询后，Search 和 Evaluate 为共享步骤；随后根据数据集是否免费分叉为两条路径：左侧绿色路径（免费数据）——直接认证 VC 签名后通过 Open Swarm 下载，全程约 5 秒、零成本；右侧红色路径（付费数据）——需经过 x402 微支付预览、ERC-8183 托管锁定资金、Seller-Only Seeding 加密传输和水印嵌入、Merkle 验证通过后释放托管资金，全程约 30 秒。两条路径最终汇聚，Agent 获得可用数据集。

- On-chain data valuation for AI gents
...
DHT 不支持语义搜索
双层索引
DHT 倒排 + Qdrant 向量；GossipSub 同步
跨源结果格式异构
统一 DatasetRecord
Croissant (JSON-LD) 扩展 + adapter
搜索质量不可控
反馈循环 + 声誉加权
EAS 链上声誉影响排序权重
查询暴露 Agent 意图
PIR
k-anonymity 混淆；可选 Tor/mixnet
新数据集传播延迟
GossipSub + DHT
5s 在线到达；2min 全网收敛
...
多步骤工作流编排
Payment Router
自动选择 x402/MPP/ERC-8183
预览与全量不一致
承诺-验证机制
Merkle Root + 统计摘要签名承诺
Agent 钱包安全
...
付费数据防泄露
三层防御
Seller-Only + 组合水印 + stake/slash
水印鲁棒性
组合水印策略
三种正交水印；抗 30% 行删除
搜索隐私
PIR + k-anonymity
DHT 混淆；可选 Tor/mixnet
隐私数据质量证明
ZKP + Compute-to-Data
本地 ZKP 证明；买家仅验证证明
传输层窃听
TLS 1.3 端到端加密
libp2p Noise + TLS 1.3 双层
...
零配置发布
文件监控 + 自动流水线
watchdir → Parquet → Merkle → VC → DHT
冷启动无 Seeder
Super-Seed + WebSeed
token 激励 + HTTP fallback
NAT 穿透
AutoNAT + Relay v2
自动检测 NAT；必要时 relay 中转
大数据集传输效率
BT v2 Merkle 分片
Rarest-First + Range Request 预览
数据集版本管理
IPNS 式可变指针
DID URI → 最新版本 CID
Seller-Only 性能瓶颈
...
[图片]
图 3：Guixu Demo 场景总览——跨源数据集发现、交易与验证

图 3 展示了 Guixu 的典型部署场景：左侧 Provider Node（Full Node）通过文件监控自动将数据集注册到 P2P 网络；中间 P2P Network 包含 Kademlia DHT、GossipSub 和 BitTorrent Swarm 三个核心组件；右侧 Agent Node（Light Node）通过 MCP 接口搜索、交易和验证数据集。外部数据源（Kaggle/HF）和区块链（Base L2）分别提供跨源聚合和链上交易保障。

本章从用户视角出发，展示 Guixu 如何在真实场景中解决具体需求。场景分为免费数据和付费数据两大类，每类下细分多种子场景。
...
IT 部门预配置 (一次性):
  ...
  # 销售部 PostgreSQL
  $ guixu init --source postgres://sales-db:5432/main
  $ guixu start --auto-register-tables  # 自动将每张表注册为数据集

  # 财务部 MySQL
  $ guixu init --source mysql://finance-db:3306/ledger
  $ guixu start --auto-register-tables

  # 物流部 ClickHouse
  $ guixu init --source clickhouse://logistics:8123/warehouse
  $ guixu start --auto-register-tables
...
数据供应商 (Provider):
  1. 部署 Full Node，设置付费数据集:
     $ guixu set-price bafybeig... --price 50.00 --currency USDC --period monthly
     $ guixu set-license bafybeig... --license commercial --terms "no-redistribution"
...
医院数据管理方 (Provider):
  1. 发布隐私数据集:
     $ guixu publish clinical_trial_rare_disease.parquet \
...
农业合作社 (Provider):
  1. IoT 网关部署 Full Node:
     $ guixu init --source mqtt://iot-gateway:1883/sensors
     $ guixu start --stream-mode --update-interval 1h
...
数据库表
免费（内网）
mDNS 发现 + 自动注册 + Range Request
D 科研数据
学术数据集
免费
Provenance Chain + EAS 时间戳 + DOI
E 金融数据
高频交易数据
$50/月
三级预购评估 + ERC-8183 + 水印
...
4.1 数据可验证技术（Data Authentication）
...
第五章 路线图
现代软件系统成长路径：
(1) 原型论文 -> 商业化系统 -> 开源社区复现版本，比如：Google File System, Bigtable
(2) 原型论文 -> 开源社区版本 -> 商业化系统，比如：Apache Spark, Postgres
[图片]

近期 (1-12 个月): 原型实现+打造开源社区+发表Demo论文
  ├─ 核心协议实现 (Rust) 并开源
  ├─ 基础 MCP Server 上线 (search + verify + trade)
  ├─ 发表 VLDB 2026 Demo Paper
  └─ 建立初始 P2P 网络 (bootstrap 节点 + 早期用户)

中期 (1-2 年): 优化用户体验与打造技术壁垒
...
远期 (3-5 年): 与时俱进
  ...
  ├─ 数据市场均衡理论
  ├─ 多模态数据集搜索
  └─ 致力于成为 AI Agent 数据基础设施的事实标准

第六章 总结
Guixu 不仅是一个工程系统，更是一个研究平台。它为数据管理、分布式系统、密码学、经济学和 AI Agent 等多个领域的交叉研究提供了实验基础。我们相信，随着 AI Agent 从实验走向生产，数据的自主发现、验证和交易将成为 Agent 基础设施的核心组成部分，而 Guixu 正是这一基础设施的有影响力的基石。
