# Guixu（归墟）：面向 AI Agent 的 P2P 数据集搜索与交易协议 — 项目报告
...
![Guixu 系统架构](../figure/architecture.svg)
...
| 场景 | 目标 | 核心方法 |
|------|------|---------|
| 免费数据 | 选出最适合当前任务的数据集 | Task Fitness Score（6 维加权） |
| 付费数据 | 判断是否值得花钱购买 | ROI 评估 + Fast-DataShapley |
| Agent Memory/Skills | 评估共享经验是否适合任务 | 能力图谱匹配（5 维加权） |

- **Task Fitness Score 权重**：schema 相关性 30% + 时间覆盖 20% + 信息增益 20% + 质量 15% + 新鲜度 10% + 去重价值 5%
...
![Guixu 端到端工作流](../figure/workflow.svg)
...
| 子问题 | 解决技术 | 实现方式 |
|--------|---------|---------|
| DHT 不支持语义搜索 | 双层索引 | DHT 倒排 + Qdrant 向量；GossipSub 同步 |
| 跨源结果格式异构 | 统一 DatasetRecord | Croissant (JSON-LD) 扩展 + adapter |
| 搜索质量不可控 | 反馈循环 + 声誉加权 | EAS 链上声誉影响排序权重 |
| 查询暴露 Agent 意图 | PIR | k-anonymity 混淆；可选 Tor/mixnet |
| 新数据集传播延迟 | GossipSub + DHT | ~5s 在线到达；~2min 全网收敛 |

#### 挑战二解决：数据可验证性

| 子问题 | 解决技术 | 实现方式 |
|--------|---------|---------|
| 数据完整性验证 | BT v2 Merkle + CID | 重算 Merkle Root 与 VC 比对 |
| 数据来源验证 | DID + VC + EAS | Ed25519 签名；可选链上锚定 |
| 质量不可证明 | 分层认证 L1-L4 | 自动→自声明→审计→ZKP |
| 隐私数据认证悖论 | Noir ZKP + 采样证明 | 本地 ZKP 电路；验证者提供采样种子 |
| 派生数据溯源 | Provenance Chain | VC 含 `derivedFrom`，形成 DAG |
| DID 信任冷启动 | 渐进式信任 | EAS 锚定提升；可信 DID 背书 |

#### 挑战三解决：自动化数据交易

| 子问题 | 解决技术 | 实现方式 |
|--------|---------|---------|
| 多步骤工作流编排 | Payment Router | 自动选择 x402/MPP/ERC-8183 |
| 预览与全量不一致 | 承诺-验证机制 | Merkle Root + 统计摘要签名承诺 |
| Agent 钱包安全 | ERC-4337 Session Key | 临时密钥 + 单笔/总限额 |
| 许可证不可机器执行 | ODRL + 分层执行 | 技术层 + 经济层 + 社会层 |
| 法币与加密混合结算 | Stripe MPP | SPT/USDC 统一在 Dashboard 结算 |

#### 挑战四解决：数据估值

| 子问题 | 解决技术 | 实现方式 |
|--------|---------|---------|
| 免费数据选择困难 | Task Fitness Score | 6 维加权评分 |
| 付费数据 ROI 不透明 | Fast-DataShapley | O(1) 推理 + 免费替代品对比 |
| 信息增益暴露 Agent 数据 | 本地计算 | 仅需 VC 中的统计摘要 |
| Shapley 值计算不可行 | Fast-DataShapley | 预训练 explainer，推理 O(1) |
| 定价操纵 | Sybil 抵抗 | 消耗 gas + 交易历史加权 + 异常检测 |
| 跨领域估值不可比 | 领域分类 + 相对估值 | 类别内排序，展示 Top X% |

#### 挑战五解决：数据隐私保护

| 子问题 | 解决技术 | 实现方式 |
|--------|---------|---------|
| 付费数据防泄露 | 三层防御 | Seller-Only + 组合水印 + stake/slash |
| 水印鲁棒性 | 组合水印策略 | 三种正交水印；抗 30% 行删除 |
| 搜索隐私 | PIR + k-anonymity | DHT 混淆；可选 Tor/mixnet |
| 隐私数据质量证明 | ZKP + Compute-to-Data | 本地 ZKP 证明；买家仅验证证明 |
| 传输层窃听 | TLS 1.3 端到端加密 | libp2p Noise + TLS 1.3 双层 |

#### 挑战六解决：去中心化数据共享

| 子问题 | 解决技术 | 实现方式 |
|--------|---------|---------|
| 零配置发布 | 文件监控 + 自动流水线 | watchdir → Parquet → Merkle → VC → DHT |
| 冷启动无 Seeder | Super-Seed + WebSeed | token 激励 + HTTP fallback |
| NAT 穿透 | AutoNAT + Relay v2 | 自动检测 NAT；必要时 relay 中转 |
| 大数据集传输效率 | BT v2 Merkle 分片 | Rarest-First + Range Request 预览 |
| 数据集版本管理 | IPNS 式可变指针 | DID URI → 最新版本 CID |
| Seller-Only 性能瓶颈 | 授权 Seeder 网络 | 委托可信节点持有加密分片 |
...
![Guixu 演示界面](../figure/demo-ui.svg)

> 图 4 展示了 Guixu 的四面板演示界面：**(A)** Provider 终端——展示零配置数据集发布的实时日志；**(B)** Agent 聊天界面——展示 Agent 自主搜索、评估和购买数据集的全过程；**(C)** 观众交互面板——用户可亲手搜索、发布数据集和触发篡改检测；**(D)** 区块链浏览器——实时展示 ERC-8183 托管交易、EAS 认证和 Merkle 验证结果。
